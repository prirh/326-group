...
public class Game {
    ...
    private String getWinningMove() {
        for(String move : moves) {
            ...
            // System.out.println("0: "+PEANUTS + ":"+peanutMove + " , " + PRETZELS + ":"+pretzelMove);
            if(PEANUTS-peanutMove>=0 && PRETZELS-pretzelMove >=0){
                if (winningMove(PEANUTS, peanutMove, PRETZELS, pretzelMove, true,1)) {
                    return move;
                }
            }
        }
     ...
     */
    private boolean winningMove(int peanuts, int peanutMove, int pretzels, int pretzelMove, boolean turn, int turns) {
        ...
        for(String move : moves) {
            ...
            if(memo.get(situation) != null) {
              ...
              continue;
                ...
                boolean outcome = winningMove(peanutsLeft, nextPeanut, pretzelsLeft, nextPretzel, !turn,turns+1);
                //System.out.println(turns+": "+peanuts + ":"+nextPeanut+ " , " + pretzels + ":"+nextPretzel);
                //  System.out.println(outcome);
                memo.put(situation, outcome);
                /* if(outcome && turn) return true;
                   else if(!outcome && !turn) return false;*/
