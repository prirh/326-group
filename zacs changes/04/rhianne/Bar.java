    ...
    public static int calc(int x2, int y2, Move[] rules,  int turn){
        ...
        if(rules[val].peanutsOperator =='>'&& rules[val].pretzelsOperator-'<'){
            for(int i=((x2-rules[val].peanuts)*rules[val].pretzels)-1; i>=0;i--){
                if(x2-((i/3)+rules[val].peanuts+1)>=0 && y2-i%3>=0){
                    if(calc(x2-((i/rules[val].pretzels)+rules[val].peanuts+1),y2-i%rules[val].pretzels),turn)==1){
        ...
        else if(calc(x2,y2-1,rules,turn)==1){
            result[0] =0;
            result[1]=1;
            return 1-turn;
        }
